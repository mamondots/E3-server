...
const getNewArrivalProducts = async (limit = 10) => {
  const result = await productModel.find().sort({ createdAt: -1 }).limit(limit);
  return result;
};

const getDiscountProducts = async (limit = 10) => {
...
const getBestSellerProducts = async (limit = 10) => {
  ...
  // If no sold products → return latest 10 products
